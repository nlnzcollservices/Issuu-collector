...
sprdsht_path = r"Y:\ndha\pre-deposit_prod\LD_working\issuu_main\assets\metadata\Issuu titles scoping for automation.xlsx"
...
def routine():
	...
	ws = wb.get_sheet_by_name("all-titles checked")#("sent_Svetlana_Oct_2021")#
	print(dir(ws))
	for i in range(ws.max_row):
		if i==271:
			...
			try:
				print(ws["C{}".format(i+1)].value)
				mms = ws["C{}".format(i+1)].value.rstrip(" ")
				print(mms)
				...
				#print(holding_id)
				publisher = ws["G{}".format(i+1)].value.rstrip(" ")
				try:
					url = ws["B{}".format(i+1)].value.rstrip(" ")
					...
					username = None
				try:
					po_line = ws["J{}".format(i+1)].value.rstrip(" ")
				except:
					po_line =None
